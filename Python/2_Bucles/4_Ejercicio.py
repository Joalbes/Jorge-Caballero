...
while entrar:  
    ...
    if opcion < 0 or opcion > 4:
        ...
        if opcion == 1:
            ...
            print('La resta de',numeroUno,'y',numeroDos,'y',numeroTres,'es',resta,'\n')
        elif opcion == 3:
            print('\nMultiplicar dos numeros')
            numeroUno = float(input('\nIngrese el primer numero: '))
            numeroDos = float(input('Ingrese el segundo numero: '))
            multiplicacion = numeroUno * numeroDos
            print('La multiplicacion entre',numeroUno,'y',numeroDos,'es',multiplicacion,'\n')
        elif opcion == 4:  
            print('\nDivisón de  dos numeros')
            numeroUno = float(input('\nIngrese el primer numero: '))
            entrarDivision = True
            while entrarDivision:
                numeroDos = float(input('Ingrese el divisor, recuerde que debe ser mayor que cero: '))
                if numeroDos == 0:
                    print('Error: Numero invalido. El numero no puede ser cero')
                else:
                    entrarDivision = False    
            division = numeroUno / numeroDos
            print('La division entre',numeroUno,'y',numeroDos,'es',division,'\n')
        elif opcion == 0: 
