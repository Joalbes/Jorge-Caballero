  ...
  while (entrarMenuNivel) {
    switch (numero) {
      ...
      default:
        ...
        numero = prompt(
          `Ingrese el nivel segun la experiencia:\n1. Beginner.\n2. Junior.\n3. Senior. `
        );
        break;
    }
  }
...
while (entrarMenu) {
  ...
  switch (opcionMenu) {
    case "1": //Ingreso numero de estudiantes
      numeroEstudiantes = validacionEntero(
        numeroEstudiantes,
    ...
    case "2": //Ingreso datos basicos del estudiante.
      if (banderaNumeroEstudiantes) {
        ...
        alert(
          'Error:No ha ingresado el número de estudiantes, Vuelva a la opción "1" del menu principal'
        );
      }
    ...
    case "3": //Mostar datos de estudiantes
      if (banderaNumeroEstudiantes == false) {
        alert(
          'Error: Ingrese primero el numero de estudiantes. Vaya a la opción "1" del menu principal.'
        );
      } else {
        if (banderaRegistroEstudiantes == false) {
          alert(
            'Error: Ingrese primero los datos b{asicos de los estudiantes. Vaya a la opción "2" del menu principal.'
          );
        } else {
          console.log("El listado de estudiantes es:");
          estudiantes.forEach((element) => {
            console.log(
              `Estudiante: ${element.nombre}.\nCódigo: ${element.codigo}.\nNivel: ${element.nivel}`
            );
          });
        }
      }
    ...
    case "4": //Registrar notas de estudiantes
      if (banderaNumeroEstudiantes == false) {
        ...
        if (banderaRegistroEstudiantes == false) {
          alert(
            'Error: Ingrese primero los datos básicos de los estudiantes. Vaya a la opción "2" del menu principal.'
          );
        } else {
          for (let j = 0; j < estudiantes.length; j++) {
            estudiantes[j].nota1 = parseFloat(
              prompt(`Ingrese la nota 1 de ${estudiantes[j].nombre}:`)
            );
            estudiantes[j].nota1 = notaValida(estudiantes[j].nota1);
            entrarNotaValida = true;

            estudiantes[j].nota2 = parseFloat(
              prompt(`Ingrese la nota 2 de ${estudiantes[j].nombre}:`)
            );
            estudiantes[j].nota2 = notaValida(estudiantes[j].nota2);
            entrarNotaValida = true;

            estudiantes[j].nota3 = parseFloat(
              prompt(`Ingrese la nota 3 de ${estudiantes[j].nombre}:`)
            );
    ...
    case "5": // Imprimir notas de estudiantes
      if (banderaNumeroEstudiantes == false) {
        ...
        if (banderaRegistroEstudiantes == false) {
          ...
          if (banderaNotas == false) {
            alert(
              'Error: Ingrese primero las notas de los estudiantes. Vaya a la opción "3" del menu principal.'
            );
          } else {
            estudiantes.forEach((el) => {
              console.log(
                `Estudiante: ${el.nombre}.\nNota1: ${el.nota1}.\nNota2: ${el.nota2}.\nNota3: ${el.nota3}.\nNota Final: ${el.promedio}.\nAprobó: ${el.aprobo}.\n`
              );
      ...
      console.log(
        "La nota promedio del grupo es: " + notaAcumulada / numeroEstudiantes
      );
